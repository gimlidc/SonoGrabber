...
IGTLinkClient::IGTLinkClient(QObject *parent) : QObject(parent)
{
    ...
    connect(this, &IGTLinkClient::startWorker, worker, &Worker::listen);
...
void IGTLinkClient::setup(const char *hostname, int port)
{
    _hostname = hostname;
    _port = port;
...
int IGTLinkClient::open(const char *hostname, int port)
{
    setup(hostname,port);
    return open();
}

int IGTLinkClient::open()
{
    int r = _socket->ConnectToServer(_hostname.toLatin1(), _port);
...
{
    if (!open()) emit startWorker(_socket);
...
void Worker::listen(igtl::Socket *socket)
{
    int resultCode;
